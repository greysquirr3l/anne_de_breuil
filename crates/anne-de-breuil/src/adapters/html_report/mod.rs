...
const TAIL: &str = "</main>\n<footer class=\"hairline\"><p>Generated by <a \
href=\"https://github.com/greysquirr3l/anne_de_breuil\">Anne de Breuil</a></p></footer>\n</body>\n</html>\n";
...
mod tests {
    ...
    fn rendered_artifact_has_zero_external_resource_references() {
        ...
        assert_every_external_href_is_a_plain_anchor_link(&html);
    }

    /// `href="http...` is allowed only on an `<a>` element -- a plain
    /// navigation link a viewer can choose to follow, which never affects
    /// whether the document renders correctly offline. `<link
    /// href="http...">` (or any other element) would mean the document
    /// depends on a network fetch to look right, so that stays forbidden;
    /// `src="http`/`url(http` above already cover every resource-loading
    /// case outright. This is the one legitimate exception: the report's
    /// own repo-attribution footer link.
    fn assert_every_external_href_is_a_plain_anchor_link(html: &str) {
        let mut rest = html;
    ...
    fn split_files_are_each_independently_self_contained() {
        ...
        assert_every_external_href_is_a_plain_anchor_link(&doc);
        assert!(
            doc.contains("base64,"),
